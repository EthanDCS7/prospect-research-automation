...
@app.command()
def score(
    input: str = typer.Option(..., "--input", help="Input targets CSV"),
    ...
    preset: Optional[str] = typer.Option(None, "--preset", help="Preset name (without .yaml)"),
    fallback_employees: Optional[int] = typer.Option(
        None, "--fallback-employees", help="If CSV lacks employees, use this value"
    ),
    ...
    """
    Config-driven scoring. Applies bands/weights/boosts from YAML.
    Writes contributions and exclusion_reason into output CSV.
    """
    ...
    # Excluded rows sink to the bottom; then by score desc
